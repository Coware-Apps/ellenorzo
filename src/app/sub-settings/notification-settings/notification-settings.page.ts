...
export class NotificationSettingsPage implements OnInit {
    ...
    ngOnInit() {
        ...
        this.firebase.setScreenName("notification-settings");
    }
    ...
    async toggleNotifications() {
        await this.app.changeConfig(
            "localNotificationsEnabled",
            !this.app.localNotificationsEnabled
        );
        ...
        this.firebase.logEvent(
            "toggle_timetable_notifications",
            this.app.localNotificationsEnabled
        );
    }
    ...
    async showScheduledNotifications() {
        ...
        allNotifications = allNotifications.sort(
            (a, b) => a.trigger.at.valueOf() - b.trigger.at.valueOf()
        );

        allNotifications.forEach(notification => {
            msg += `<li>${notification.title} - ${this.fDate.formatDateWithZerosAndDots(
                notification.trigger.at
            )} ${this.fDate.getTime(notification.trigger.at)}</li>`;
        });

        let alert = await this.alertCtrl.create({
            header: `${this.translator.instant(
                "pages.notification-settings.scheduledNotifications.header"
            )} (${allNotifications.length}db)`,
            subHeader: this.translator.instant(
                "pages.notification-settings.scheduledNotifications.subHeader"
            ),
            message: `<ol>${msg}</ol>`,
        });
