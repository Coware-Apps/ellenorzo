...
export class TestsPage implements OnInit {
    ...
    private loadData(forceRefresh: boolean = false, event?) {
        this.userManager.currentUser
            .getAsyncAsObservableWithCache<[Test[]]>(
                [
                    {
                        name: "getTestsV3",
                        cacheKey: "v3Tests",
            ...
            .subscribe({
                next: d => {
                    if (d[0]) {
                        this.testsByMonth = this.collapsifyService.collapsifyByMonths(
                            d[0],
                            "Datum"
                        );
                    }
    ...
    private setComponentState() {
        if (!this.testsByMonth) {
            this.componentState = "error";
        } else if (this.testsByMonth.findIndex(md => md.data && md.data.length > 0) == -1) {
