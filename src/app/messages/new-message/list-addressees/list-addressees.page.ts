...
import { AddresseeListItem } from "src/app/_models/administration/addresseeListItem";
...
export class ListAddresseesPage implements OnInit {
    ...
    constructor(public app: AppService, private dataService: DataService) {}
    ...
    removeAddressee(a: AddresseeListItem) {
        ...
        for (let i = 0; i < this.currentAddresseeList.length; i++) {
            if (a.kretaAzonosito == this.currentAddresseeList[i].kretaAzonosito) {
                this.currentAddresseeList.splice(
                    this.currentAddresseeList.indexOf(this.currentAddresseeList[i]),
                    1
                );
