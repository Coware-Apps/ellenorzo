...
import { AddresseeType } from "src/app/_models/administration/addresseeType";
...
} from "src/app/_models/administration/addresseeListItem";
import { ThemeService } from "src/app/_services/theme.service";
import { AddresseeGroup } from "src/app/_models/administration/adresseeGroup";
...
export class AddresseeSelectorPage implements OnInit {
    ...
    ionViewWillEnter() {
        if (this.dataService.getData("currentAddresseeList") != null) {
            this.finalAddresseeList = this.dataService.getData("currentAddresseeList");
            ...
            if (this.filter != null) {
                this.filteredAddresseeList = this.displayAddresseeList.filter(x =>
                    this.diacriticsHelper
                        ...
                        .includes(this.diacriticsHelper.removeDiacritics(this.filter.toLowerCase()))
                );
            } else {
        ...
        if (this.codeRequestDict[toCategory] != null) {
            ...
            if (toCategory != "GONDVISELOK" && toCategory != "TANULOK") {
                ...
                try {
                    ...
                    this.displayGroups = this.groups.filter(g => !g.osztalyKretaAzonosito);
                } catch (error) {
                    throw error;
                } finally {
                    this.sans = false;
    ...
    async subCategoryChanged(subCategoryName: "byClasses" | "byGroups", event) {
        ...
        if (subCategoryName == "byClasses") {
            ...
            this.displayGroups = this.groups.filter(g => g.osztalyKretaAzonosito == toSubCategory);
        ...
        if (list != null) {
            for (let i = 0; i < list.length; i++) {
                for (let j = 0; j < this.finalAddresseeList.length; j++) {
                    if (list[i].kretaAzonosito == this.finalAddresseeList[j].kretaAzonosito) {
                        list[i].isAdded = true;
                    }
                }
            }
        ...
        this.filteredAddresseeList = this.displayAddresseeList.filter(x => {
            if ($event.target.value) {
                return this.diacriticsHelper
                    .removeDiacritics(this.getName(x).toLowerCase())
                    .includes(
                        this.diacriticsHelper.removeDiacritics($event.target.value.toLowerCase())
                    );
            } else return true;
        });
    }
