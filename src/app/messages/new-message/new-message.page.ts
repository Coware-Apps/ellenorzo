...
} from "src/app/_models/administration/message";
...
export class NewMessagePage implements DirtyPage, OnDestroy {
            ...
            if (p != null) {
                if (p.replyDataKey != null) {
                    ...
                    this.prevMsgText =
                        ...
                        `${this.translator.instant("pages.new-message.sentAtName")}: ${
                            this.fDate.formatDateWithZerosAndDots(prevMsg.uzenet.kuldesDatum) +
                            " " +
                            this.fDate.getTime(new Date(prevMsg.uzenet.kuldesDatum))
                        }<br>` +
    ...
    ngOnDestroy(): void {
        ...
        this.app.swipeGestureEnabled = true;
        this.querySubscription.unsubscribe();
    }

    ionViewWillEnter() {
        this.menuCtrl.enable(false);
        this.app.swipeGestureEnabled = false;
    ...
    getAddresseeString() {
        ...
        if (returnval.length > 30) {
            return (
                returnval.substring(0, 30) +
                `... (${this.addresseeList.length} ${this.translator.instant(
                    "pages.new-message.piecesName"
                )})`
            );
        } else {
            return returnval;
            ...
            if (!this.showProgressBar) {
                ...
                if (this.prevMsgId == null) {
                    ...
                    try {
                        ...
                        this.prompt.toast(
                            this.translator.instant("pages.new-message.messageSentText"),
                            true
                        );
