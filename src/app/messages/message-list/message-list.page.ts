...
import { MessageListItem } from "src/app/_models/administration/message";
...
export class MessageListPage implements OnDestroy, OnInit {
    ...
    public componentState: "loading" | "loaded" | "empty" | "error" | "loadedProgress" | "login" =
        "loading";
    ...
    public async loadMessages(forceRefresh: boolean = false, event?) {
        ...
        this.userManager.currentUser
            ...
            .pipe(takeUntil(this.willLeaveUnsubscribe$))
            .subscribe({
                next: d => {
                    if (d[0]) {
                        this.messages = d[0];
                        this.messages = this.messages.sort(
                            (a, b) =>
                                new Date(b.uzenetKuldesDatum).valueOf() -
                                new Date(a.uzenetKuldesDatum).valueOf()
                        );
    ...
    public openNewMsgPage() {
        this.firebase.logEvent("compose_new_message");
    ...
    public onSearchChange(event) {
        ...
        this.toBeDisplayed = this.messages.filter(x => {
            const search = this.diacriticsHelper.removeDiacritics(
                event.detail.value.toLocaleLowerCase()
            );

            // subject
            if (
                this.diacriticsHelper
                    .removeDiacritics(x.uzenetTargy.toLocaleLowerCase())
                    .includes(search)
            )
                return true;
            ...
            if (
                ...
                (this.diacriticsHelper
                    .removeDiacritics(x.uzenetFeladoNev.toLocaleLowerCase())
                    .includes(search) ||
                    this.diacriticsHelper
                        .removeDiacritics(x.uzenetFeladoTitulus.toLocaleLowerCase())
                        .includes(search))
            )
                return true;
            ...
            //             .includes(this.diacriticsHelper.removeDiacritics(search))
            //     )
            //         return true;
            // }
        });
    ...
    async binSelected(action: "put" | "remove") {
        ...
        if (action == "put") {
            this.prompt.toast(
                this.translator.instant("pages.message-list.successfullyDeletedText"),
                true
            );
        } else {
            this.prompt.toast(
                this.translator.instant("pages.message-list.successfullyRestoredText"),
                true
            );
        }
        ...
        this.unselectAll();
        this.componentState = "loadedProgress";
        await this.userManager.currentUser.clearUserCacheByCategory("outboxMessageList");
        await this.userManager.currentUser.clearUserCacheByCategory("inboxMessageList");
        await this.userManager.currentUser.clearUserCacheByCategory("deletedMessageList");
