...
import { Note, Absence, evaluation } from "../_models/kreta-v2/student";
...
export interface UniversalSortedData {
    index: number;
    header: string;
    data: any[] | Note[] | Absence[] | evaluation[] | Lesson[] | Homework[];
...
export class CollapsifyService {
        ...
        months.forEach(month => {
            ...
            sortedData.push({
                ...
                firstEntryCreatingTime: new Date(
                    monthlyData[monthlyData.length - 1][sortBy]
                ).valueOf(),
                showAll: true,
            });
            i++;
    ...
    collapsifyByDates(
        ...
        dates.forEach(date => {
            ...
            sortedData.push({
                ...
                firstEntryCreatingTime: new Date(
                    dataOnCurrentDate[dataOnCurrentDate.length - 1][sortBy]
                ).valueOf(),
                showAll: true,
            });
            i++;
    ...
    collapsifyByNames(
        ...
        names.forEach(name => {
            ...
            sortedData.push({
                ...
                firstEntryCreatingTime: new Date(
                    dataWithCurrentName[dataWithCurrentName.length - 1][sortBy]
                ).valueOf(),
                showAll: true,
            });
            i++;
    ...
    collapsifyEvaluationsBySubjects(data: Evaluation[]): UniversalSortedData[] {
        //sortBy, groupBy must reference a date / datestring field!
        let names: string[] = [];
        let sortedData: UniversalSortedData[] = [];

        data.forEach(element => {
            let groupByName = element.Tantargy.Nev;
            if (!names.includes(groupByName)) {
                names.push(groupByName);
            }
        });
        ...
        names.forEach(name => {
            ...
            dataWithCurrentName.sort(
                (a, b) =>
                    new Date(b.KeszitesDatuma).valueOf() - new Date(b.KeszitesDatuma).valueOf()
            );
            ...
            sortedData.push({
                ...
                data: dataWithCurrentName,
                firstEntryCreatingTime: new Date(
                    dataWithCurrentName[dataWithCurrentName.length - 1].KeszitesDatuma
                ).valueOf(),
                showAll: true,
            });
            i++;
