...
export class FormattedDateService {
    ...
    formatDateWithZerosAndDots(d: any) {
        ...
        return (
            this.addZeroToNumberByLength(date.getFullYear()) +
            ". " +
            this.addZeroToNumberByLength(month) +
            ". " +
            this.addZeroToNumberByLength(dateStringSplitted[2]) +
            "."
        );
    }
    ...
    getWeekFirst(extraWeeks: number = 0) {
        ...
        return (
            ...
            month +
            "-" +
            parseInt(returndate.toString().split(" ")[2])
        );
    }
    ...
    getWeekFirstByDate(date: string | Date) {
        const d = new Date(date);
        const day = d.getDay();
        return this.formatDate(
            new Date(d.getFullYear(), d.getMonth(), d.getDate() + (day == 0 ? -6 : 1) - day)
        );
    }
    getWeekLastByDate(date: string | Date) {
        const d = new Date(date);
        var day = d.getDay();
        let returndate = new Date(
            d.getFullYear(),
